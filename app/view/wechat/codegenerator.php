...
switch($type) {
 ...
 /*
		$myfile = fopen("ab.txt", "w");
              // $txt = $url.'------'.$mid;
               fwrite($myfile,  $result['event'].'-----'.$result['key']);
                fclose($myfile);
	...
	/*	$con = mysql_connect("39.100.129.160:3306","root","root");
		...
		while($row = mysql_fetch_array($result))
			...
			echo "start";
			$record = "";
			for($scene_id = 76000;$scene_id < 78000;$scene_id++)
			{
			...
			break;
			  ...
			  echo substr($result['url'],strpos($result['url'], '/02') + 3);
			$record = $record.',"'.substr($result['url'],strpos($result['url'], '/02') + 3).'"';		
			}
