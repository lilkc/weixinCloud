...
class Wechat
{
	...
	const MENU_CONTACT_US = 'menuContactUs';
	const MENU_READ_US = 'menuReadXingMai';
	const MENU_ABP_MEAN = 'menuABPmean';
	const MENU_WBP_BLUETOOTH = 'menuBluetooth';
	const MENU_WBP_WIFI = 'menuwifi';
	const MENU_WBP_DOCTOR = 'menuDoctor';
	const MENU_WBP_GPRS = 'menugprs';
	const MENU_ABP = 'menuAbp';
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function savePressure($userid,$sys,$dia,$hr){
		$con = mysql_connect("39.100.129.160:3306","root","Hingmedroot");
		if (!$con)
		{
		...
		$temptime = time();
		mysql_query("INSERT INTO bloodpressure (userID,sys,dia,hr,time) VALUES ('{$userid}','{$sys}','{$dia}','{$hr}','{$temptime}')");
		mysql_close($con);
	}

	/**
	*从数据库中读取access_token，如果数据库中没有token,或者token将过期，就申请新的token.
	*/
	public function getAccessToken()
	{
		...
		$result = mysql_query("SELECT * FROM tokenrecord WHERE appid ='$this->appid'" );
		if($result)
		{
			while($row = mysql_fetch_array($result))
				{ 
				$this->access_token = $row['token'];
				$expiretTime = $row['time'];				
				if( time() < $expiretTime)
					{
					  mysql_close($con);					  
					...
					if ($result)
						...
						$expire = $json['expires_in'] ? intval($json['expires_in'])-300 + time() : 3300 + time();
						mysql_query("UPDATE tokenrecord SET token = '$this->access_token' WHERE appid = '$this->appid'");
						mysql_query("UPDATE tokenrecord SET time = '$expire' WHERE appid = '$this->appid'");
						mysql_close($con);
						return $this->access_token;
						}
					mysql_close($con);
			...
			if ($result)
			  ...
			  $this->access_token = $json['access_token'];
			  $expire = $json['expires_in'] ? intval($json['expires_in'])-300 + time() : time() + 3300;
			  //$this->setCache($authname,$this->access_token,$expire);
			  mysql_query("INSERT INTO tokenrecord (appid,token,time) VALUES ('{$this->appid}','{$this->access_token}','{$expire}')");
			  mysql_close($con);
			  return $this->access_token;
			  }
			  mysql_close($con);
		...
		{ 
				...
				if( time() < $expiretTime)
					...
					if ($result)
						...
						$this->access_token = $json['access_token'];
						$expire = $json['expires_in'] ? intval($json['expires_in'])-300 : 3300;
						$this->setCache($authname,$this->access_token,$expire);
						mysql_query("UPDATE tokenrecord SET token = '$this->access_token' WHERE appid = '$appid'");
						mysql_query("UPDATE tokenrecord SET time = '$expire' WHERE appid = '$appid'");
						return $this->access_token;
						}
					return false;
				}
	...
	public function createMenu($data){	
		$this->access_token = $this->getAccessToken();
		if (!$this->access_token) return false;	
		$tempJson = self::json_encode($data);
		echo $tempJson;
		$result = $this->http_post(self::API_URL_PREFIX.self::MENU_CREATE_URL.'access_token='.$this->access_token,$tempJson);
		echo $result;
		if ($result)
		{
			$json = json_decode($result,true);
